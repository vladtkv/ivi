...
public class MainModel {
	...
	public void accessChannelVideoList(int index) {
		String channelID = subscriptionList.get(index).getSnippet()
		...
		if (channelID.compareTo(currentChannelID) != 0) {
			...
			videoList = (ArrayList<SearchResult>) getVideoList(subscriptionList
					.get(index).getSnippet().getResourceId().getChannelId());
	...
	public List<Subscription> getSubscriptionList() {
		try {
			YouTube.Subscriptions.List subscritpionRequest = youtube
					.subscriptions().list("contentDetails");
			subscritpionRequest.setMine(true);
			subscritpionRequest
			.setFields("items/contentDetails,nextPageToken,pageInfo");
			SubscriptionListResponse subscriptionResult = subscritpionRequest
					.execute();
			...
			if (subscriptionList != null) {
				...
				YouTube.Subscriptions.List subscrListItemRequest = youtube
						.subscriptions().list("id,contentDetails,snippet");
				// Only retrieve data used in this application, thereby making
				// the application more efficient. See:
				// https://developers.google.com/youtube/v3/getting-started#partial
				subscrListItemRequest
				.setFields("items(snippet/title,snippet/publishedAt,snippet/resourceId/channelId),nextPageToken,pageInfo");
				...
				do {
					...
					SubscriptionListResponse subscriptionItemResult = subscrListItemRequest
							.execute();
					subscritionItemList.addAll(subscriptionItemResult
							.getItems());
	...
	public List<SearchResult> getVideoList(String channelID) {
		try {
			...
			searchRequest
			.setFields("items/snippet,items/id/videoId,nextPageToken,pageInfo");
