...
public class VideoListView {
	...
	public VideoListView(MainModel mm) {
		...
		frame = new Frame(Program.PROGRAM_CAPTION_STRING, new Dimension(725,
				620), 0, JFrame.DISPOSE_ON_CLOSE, null);
		// create channel table
		Object[] subscriptionTableColumn = { "Video", "Published" };
		videoTableModel = new NonEditableVideoTableModel(null,
		...
		videoTable.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if (e.getButton() == 1 && e.getClickCount() == 2) {
					if (!mainModel.openBrowser(videoTable
							.convertRowIndexToModel(videoTable.getSelectedRow()))) {
						JOptionPane
								.showMessageDialog(
										null,
										"There was an error while openning video on YouTube!",
										Program.PROGRAM_CAPTION_STRING,
		...
		videoTable.getColumnModel().getColumn(1)
				.setCellRenderer(new TableDateCellRenderer());
