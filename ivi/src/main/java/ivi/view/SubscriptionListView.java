...
public class SubscriptionListView {
	...
	private final String DETAILED_LABEL = new String(
			"<html><i><u>Show detailed</u></i></html>");
	...
	public void init() {
		frame = new Frame(Program.PROGRAM_CAPTION_STRING, new Dimension(535,
				520), 0, JFrame.EXIT_ON_CLOSE, null);
		// create channel table
		Object[] subscriptionTableColumn = { "Subscrition", "" };
		subscriptionTableModel = new NonEditableTableModel(null,
		...
		subscriptionTable.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if (e.getButton() == 1 && e.getClickCount() == 2) {
					mainModel.accessChannelVideoList(subscriptionTable
							.getSelectedRow());
				}
			}
		});
	...
	public boolean addSubscriptionData(Subscription subscrition) {
		if (subscrition != null) {
			Object[] data = { subscrition.getSnippet().getTitle(),
					DETAILED_LABEL };
